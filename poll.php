...
class WikiPoll
{
  ...
  function renderPoll($input) {
    ...
    $authorized=0;
    if (strpos($lines[1],"AUTHORIZED")!==false){  // Important: "!==false" needs here
      $authorized=1;
      unset($lines[1]); $lines = array_values($lines);  // strip first line
    }
...
EOT;
    ...
    if (trim($lines[1])=="ALTERNATIVE"){
      $lines[1]="POINTS 1";  // alternative selection is equiivalent to vote with 1 point
    }

    $poll_points=0;
    if (strpos($lines[1],"POINTS")!==false){ // If line like POINTS 
      $terms = explode(' ',trim($lines[1]));
      if (sizeof($terms)>1 && is_numeric($terms[sizeof($terms)-1])){
        $poll_points=$terms[sizeof($terms)-1]; // Getting the number of points
      }
      unset($lines[1]); $lines = array_values($lines);  // strip first line
    }
    ...
    if (!empty($_POST['poll-ID'])
          && ($_POST['poll-ID']==$ID)
          && (($user_votes_count<$poll_points && $poll_points>0)||($user_votes_count==0 && $poll_points<=0))
          && !empty($_POST['answers'])
       )  
    {
      $user_answers=$_POST['answers'];
      if (!is_array($user_answers)){ //Just one answer
    ...
    if ( ($user_votes_count>=$poll_points && $poll_points>0)|| ($user_votes_count>0 && $poll_points<=0))
    {
      // Show results.
...
EOT;
      ...
      $str = <<<EOT
      <a name="poll-{$ID}"><p><b>$question</b></p></a>
EOT;
